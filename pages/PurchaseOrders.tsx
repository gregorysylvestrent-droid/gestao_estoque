...
interface QuotationDraftSnapshot {
  itemQuoteForms: Record<string, ItemQuoteForm[]>;
  itemVisibleQuoteForms: Record<string, number>;
  itemVendorSearch: Record<string, string[]>;
  selectedSlotByItem: Record<string, number>;
  updatedAt: string;
}
              ...
              return (
                <div key={idx} className={`flex flex-col items-center text-center space-y-4 max-w-[120px] group transition-all duration-300 ${isActive ? 'scale-110' : 'hover:scale-105'}`
                  ...
                  <div className="space-y-1" >
                    <p className={
                      `text-[10px] font-black uppercase tracking-tight transition-colors duration-300 ${isActive ? 'text-blue-600 dark:text-blue-400' :
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800" >
            {/* Log de Aprovação/Rejeição do Histórico */}
            {
              (order.approvalHistory || []).slice().reverse().map((log, lidx) => {
                ...
                return (
                  <tr key={`log-${lidx}`
                  } className={isRejected ? "bg-red-50/30 dark:bg-red-900/10" : isStatusChange ? "bg-blue-50/30 dark:bg-blue-900/10" : "bg-emerald-50/30 dark:bg-emerald-900/10"} >
...
export const PurchaseOrders: React.FC<PurchaseOrdersProps> = ({
  ...
  const [selectedSlotByItem, setSelectedSlotByItem] = useState<Record<string, number>>({});
  const [isItemsToQuoteExpanded, setIsItemsToQuoteExpanded] = useState(false);
  const [isLastPurchasesExpanded, setIsLastPurchasesExpanded] = useState(false);
  ...
  const getQuotationDraftStorageKey = (orderId: string) => `purchase-order-quotation-draft:${orderId}`;

  const loadQuotationDraftSnapshot = (orderId: string): QuotationDraftSnapshot | null => {
    try {
      const raw = window.localStorage.getItem(getQuotationDraftStorageKey(orderId));
      if (!raw) return null;
      const parsed = JSON.parse(raw) as QuotationDraftSnapshot;
  ...
  const saveQuotationDraftSnapshot = (orderId: string) => {
    try {
      const payload: QuotationDraftSnapshot = {
        itemQuoteForms,
        itemVisibleQuoteForms,
        itemVendorSearch,
        selectedSlotByItem,
        updatedAt: new Date().toISOString(),
      };
      window.localStorage.setItem(getQuotationDraftStorageKey(orderId), JSON.stringify(payload));
    } catch {
      // noop
    }
  };

  const removeQuotationDraftSnapshot = (orderId: string) => {
    try {
      window.localStorage.removeItem(getQuotationDraftStorageKey(orderId));
    } catch {
      // noop
    }
  };

  const initializeItemQuoteState = (order: PurchaseOrder, preferDraft = true) => {
    if (preferDraft) {
      const draft = loadQuotationDraftSnapshot(order.id);
      if (draft) {
        const searchOpenByItem = order.items.reduce<Record<string, boolean[]>>((acc, item) => {
          acc[item.sku] = Array.from({ length: 5 }, () => false);
          return acc;
        }, {});

        setItemQuoteForms(draft.itemQuoteForms || {});
        setItemVisibleQuoteForms(draft.itemVisibleQuoteForms || {});
        setItemVendorSearch(draft.itemVendorSearch || {});
        setItemVendorSearchOpen(searchOpenByItem);
        setSelectedSlotByItem(draft.selectedSlotByItem || {});
        return;
      }
    }
    ...
    const displayedQuoteIdsByItem: Record<string, string[]> = {};
    ...
    const selectedSlots = order.items.reduce<Record<string, number>>((acc, item) => {
      ...
      const displayedQuoteIds = displayedQuoteIdsByItem[item.sku] || [];
      const preferredIndex = itemSelectedQuoteId
        ? displayedQuoteIds.findIndex((quoteId) => quoteId === itemSelectedQuoteId)
        : -1;
    ...
    setItemVendorSearchOpen(searchOpenByItem);
    setSelectedSlotByItem(selectedSlots);
  };
  ...
  const handleOpenQuotationModal = (order: PurchaseOrder) => {
    ...
    initializeItemQuoteState(order, true);
    setSelectedQuoteId('');
  ...
  const handleOpenAnalyzeQuotation = (order: PurchaseOrder) => {
    ...
    initializeItemQuoteState(order, false);
  ...
  const buildQuotesFromForms = () => {
    ...
    const now = Date.now();
    const quotesByVendor = new Map<string, { entries: Array<{ item: PurchaseOrder['items'][number]; form: ItemQuoteForm }>; firstIndex: number }>();
    const selectedVendorByItem = new Map<string, string>();

    quotingPO.items.forEach((item) => {
      ...
      for (let slotIndex = 0; slotIndex < visibleSlots; slotIndex += 1) {
        ...
        const hasVendor = Boolean(form.vendorId);
        const hasValue = total > 0;
        const isComplete = hasVendor && hasValue;
        const isPartial = hasVendor !== hasValue;

        if (isPartial) {
          throw new Error(`No item ${item.name}, preencha fornecedor e valor para os campos informados.`);
        }

        if (!isComplete) continue;

        if (selectedSlotByItem[item.sku] === slotIndex && form.vendorId) {
          selectedVendorByItem.set(item.sku, form.vendorId);
        }

        const current = quotesByVendor.get(form.vendorId) || { entries: [], firstIndex: slotIndex };
        current.entries.push({ item, form });
        current.firstIndex = Math.min(current.firstIndex, slotIndex);
        quotesByVendor.set(form.vendorId, current);
      }
    });

    const quotes: Quote[] = Array.from(quotesByVendor.entries()).map(([vendorId, payload], index) => {
      const vendorName = vendors.find((vendor) => vendor.id === vendorId)?.name || '';
      const totalValue = payload.entries.reduce((sum, { form }) => sum + parseQuoteMoney(form.totalValue), 0);
      const validUntil = payload.entries.find(({ form }) => form.validUntil)?.form.validUntil
        || formatDatePtBR(new Date(now + 30 * 24 * 60 * 60 * 1000), '--/--/----');
      const notes = Array.from(new Set(payload.entries.map(({ form }) => String(form.notes || '').trim()).filter(Boolean))).join(' | ');

      return {
        id: `Q-${vendorId}-${now + index}`,
        vendorId,
        vendorName,
        items: payload.entries.map(({ item, form }) => {
  ...
  const handleSubmitQuotations = () => {
    ...
    let result: ReturnType<typeof buildQuotesFromForms>;
    try {
      result = buildQuotesFromForms();
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Existem campos inválidos nas cotações.';
      alert(message);
      return;
    }
  ...
  const handleSaveAndSendToApproval = async () => {
    if (!quotingPO) return;
    ...
    if (result.quotes.length === 0) {
      alert('Preencha pelo menos 1 cotação válida para enviar para aprovação.');
      return;
    }
  ...
  const handlePrintOrderDetailSheet = (order: PurchaseOrder) => {
    ...
    const selectedQuote = (order.quotes || []).find((quote) => quote.id === order.selectedQuoteId)
      || (order.quotes || []).find((quote) => quote.isSelected)
      || null;

    const resolveQuoteForItem = (sku: string) => {
      if (selectedQuote?.items?.some((entry) => entry.sku === sku && Number(entry.unitPrice || 0) > 0)) {
        return selectedQuote;
      }

      const recommendedByItemId = getRecommendedQuoteIdByItem(order, sku);
      if (recommendedByItemId) {
        const recommended = (order.quotes || []).find((quote) => quote.id === recommendedByItemId);
        if (recommended) return recommended;
      }

      return (order.quotes || []).find((quote) => quote.items?.some((entry) => entry.sku === sku && Number(entry.unitPrice || 0) > 0)) || null;
    };
    ...
    const supplierLabel = uniqueVendors.length > 1
      ...
      : (uniqueVendors[0] || selectedQuote?.vendorName || order.vendor || '-');

    const itemRows = computedRows
      .map((row, index) => `
          <tr>
            <td>${index + 1}</td>
            ...
            <td class="ta-right">${formatCurrencyBRL(row.unit)}</td>
            <td class="ta-right">${formatCurrencyBRL(row.subtotal)}</td>
          </tr>
        `)
      .join('');
      ...
      <body>
        <main class="sheet">
          ...
          <section class="meta-grid">
            ...
            <div><strong>Fornecedor:</strong> ${escapeHtml(supplierLabel)}</div>
      ...
      <div className="px-10 pb-10 pt-4 space-y-8" >
        {
          quotingPO.items.map((item, itemIndex) => {
            const visibleSlots = Math.max(1, Math.min(5, itemVisibleQuoteForms[item.sku] ?? 1));
            const filledSlotTotals = Array.from({ length: visibleSlots }).map((_, slotIndex) => ({
              slotIndex,
              total: parseQuoteMoney(getItemForm(item.sku, slotIndex).totalValue),
            })).filter((entry) => entry.total > 0);
            const suggestedQuoteSlot = filledSlotTotals.length > 0
              ? filledSlotTotals.reduce((best, current) => (current.total < best.total ? current : best)).slotIndex
              : -1;
            const selectedQuoteSlot = selectedSlotByItem[item.sku] ?? -1;

            return (
              <div key={`${item.sku}-${itemIndex}`
              } className="rounded-3xl border border-slate-100 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-900/40 p-6 space-y-5" >
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-5" >
                  {
                    Array.from({ length: visibleSlots }).map((_, slotIndex) => {
                      ...
                      const isSlotComplete = Boolean(form.vendorId && parseQuoteMoney(form.totalValue) > 0);
                      const isManuallySelected = selectedQuoteSlot === slotIndex;
                      const isSuggestedQuote = suggestedQuoteSlot === slotIndex;

                      return (
                        <div key={`${item.sku}-slot-${slotIndex}`
                        } className={`space-y-4 p-5 rounded-3xl border-2 ${theme.container}`} >
                          <div className="flex items-center justify-between" >
                            ...
                            <div className="flex items-center gap-2" >
                              ...
                              {
                                slotIndex < visibleSlots && isSlotComplete && (
                                  <button
                                    type="button"
                                    onClick={() => setSelectedSlotByItem((prev) => ({ ...prev, [item.sku]: slotIndex }))
                                    }
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800" >
              {
                paginatedFilteredOrders.length > 0 ? paginatedFilteredOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-colors group" >
                    ...
                    <td className="px-8 py-5 text-center" >
                      <span className={
                        `text-[9px] font-black px-2 py-0.5 rounded ${order.priority === 'critico'
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-300" >
            <div className="bg-white dark:bg-slate-900 w-full max-w-5xl rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-500 border border-slate-100 dark:border-slate-800" >
              <div className="p-10 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between" >
                ...
                } className="size-12 flex items-center justify-center bg-slate-100 dark:bg-slate-800 rounded-2xl text-slate-400 hover:text-red-500 transition-all" >
                          ...
                          return (
                            <div key={idx} className="flex items-center justify-between p-6 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl shadow-sm group hover:border-primary transition-all" >
                              ...
                              <div className="w-32 text-right" >
                                ...
                                <button onClick={() => setItemsList(prev => prev.filter((_, i) => i !== idx))
                                } className="text-[9px] font-black text-red-500 uppercase mt-1 opacity-0 group-hover:opacity-100 transition-all hover:underline" > Remover </button>
                              </div>
                            </div>
                          );
                        })}
              ...
              <div className="p-10 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between sticky top-0 bg-white dark:bg-slate-900 z-10" >
                <div className="flex-1" >
                  ...
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1" >
                    {quotationMode === 'analyze' ? `Revisão do Pedido ${quotingPO.id}` : `Pedido ${quotingPO.id}`}
                  </p>
                </div>
...

                  {
                    quotationMode === 'edit' ? (
                      <div className="flex gap-3" >
                        ...
                        <button
                          onClick={handleSaveAndSendToApproval}
                          className="px-8 py-3 bg-purple-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.15em] shadow-xl shadow-purple-500/20 hover:bg-purple-700 transition-all active:scale-95"
                        >
              ...
              <div className="px-10 mb-2" >
                <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-6 border border-slate-100 dark:border-slate-800" >
                  <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4 mb-4" >
                    <div className="flex items-center gap-3" >
                      ...
                      <button
                        type="button"
                        onClick={() => setIsItemsToQuoteExpanded((prev) => !prev)}
                        className="px-3 py-1 rounded-lg border border-slate-300 text-[10px] font-black uppercase tracking-widest text-slate-600 bg-white dark:bg-slate-900 dark:border-slate-700"
                      >
                        {isItemsToQuoteExpanded ? 'Recolher' : 'Expandir'}
                      </button>
                    </div>
                    <div className="space-y-2" >
                ...
                <div className="mt-6 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-2xl p-6 border-2 border-blue-200 dark:border-blue-800" >
                  <div className="flex items-center justify-between gap-2 mb-4" >
                    ...
                    <button
                      type="button"
                      onClick={() => setIsLastPurchasesExpanded((prev) => !prev)}
                      className="px-3 py-1 rounded-lg border border-blue-300 text-[10px] font-black uppercase tracking-widest text-blue-700 bg-white dark:bg-slate-900 dark:border-blue-700"
                    >
                      {isLastPurchasesExpanded ? 'Recolher' : 'Expandir'}
                    </button>
                  </div>
                  {
                    isLastPurchasesExpanded && (
                      <div className="space-y-3" >
                        {
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in" >
            <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[3.5rem] shadow-2xl p-10 animate-in zoom-in-95 border border-slate-100 dark:border-slate-800" >
              <div className="flex items-center justify-between mb-8" >
                ...
                <button onClick={() => { setIsSendModalOpen(false); setVendorOrderNum(''); }
                } className="size-12 flex items-center justify-center bg-slate-100 dark:bg-slate-800 rounded-2xl text-slate-400 hover:text-red-500 transition-all" >
                  <svg xmlns="http://www.w3.org/2000/svg" className="size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" >
                    <path d="M18 6 6 18" />
                    <path d="m6 6 12 12" />
